...
def collect_query_paths(parent_dir: str) -> List[str]:
    ...
    logging.info(f"Found {len(sql_files)} sql script at location {parent_dir}")
...
def execute_query(con: DuckDBPyConnection, query: str) -> str:
    con.execute(query)
...
def setup_database(database_path: str, ddl_query_parent_dir: str) -> None:
    ...
    for query_path in query_paths:
        ...
        logging.info(f"Executing query from {query_path}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="CLI tool to setup a destroy a database")

    # To prevent sommeone from writing create and destroy in the same sentence
